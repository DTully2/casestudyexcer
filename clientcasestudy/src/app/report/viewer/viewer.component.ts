...
export class ViewerComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.reportExpenses = new Array<Product>();
  ...
  onPickVendor(): void {
    this.formSubscription = this.generatorForm
      ...
      ?.valueChanges.subscribe((val) => {
        ...
        this.loadVendorReport(this.selectedVendor?.id);
        this.pickedProduct = false;
        this.hasProducts = false;
  ...
  onPickReport(): void {
    const productSubscription = this.generatorForm
      ...
      //     this.total = 0.0;
           this.reportExpenses.forEach((exp) => (this.total += exp.costprice));
       });
    this.formSubscription?.add(productSubscription); // add it as a child, so all can bedestroyed together
  } 
  onPickProduct(): void {
...
loadVendorReport(id: number): void {
  // expenses aren't part of the page, so we don't use async pipe here
